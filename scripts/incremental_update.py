...
from src.connectors.arxiv import fetch_category, OB_KEYWORDS
...
ARXIV_CATEGORIES = ["cs.AI", "cs.CL", "cs.LG", "cs.HC", "cs.CY", "cs.SI", "stat.ML", "econ.GN"]
...
def step_fetch_english_incremental():
    """对每本英文期刊，按 Crossref from-index-date + OpenAlex from-publication-date 抓新论文。"""
    ...
    try:
        for j in english_journals():
            for src_name, fetch in [
                ("crossref",
                 lambda: cr.fetch_works_by_issn(j["issn"], from_index_date=since)),
                ("openalex",
                 lambda: oa.fetch_works_by_source(j["openalex_source_id"], from_date=since) if j.get("openalex_source_id") else iter([])),
            ]:
...
def step_llm():
    ...
    print("\n=== LLM 双打分 ===")
    llm_score_run(batch_size=12, n_workers=llm_workers, candidate_ids=candidate_ids)
