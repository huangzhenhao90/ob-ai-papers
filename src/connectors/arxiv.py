"""
arXiv connector：用 query API 按 [分类 + OB/营销/管理关键词] 历史回溯。

设计要点：
- arXiv 没有 OB/管理学子分类，必须用关键词预过滤（与期刊抓取不同）
- query 语法: cat:cs.HC AND (abs:team OR abs:employee OR ...)
...
# OB/营销/管理 强信号关键词（只保留 arXiv 里少见但 OB 里常见的词）
# 砍掉在 AI 论文里被滥用的词：team, collaboration, decision making, consumer,
# innovation, customer, performance, brand 等。
OB_KEYWORDS = [
    # 工作 / 雇佣（核心强信号）
    "employee", "employees", "workforce", "workplace",
    "manager-employee", "subordinate",
    "hiring", "recruitment", "recruiting", "job applicant",
    "performance review", "performance appraisal",
    # HR / 劳动
    "human resource management", "HR analytics", "people analytics",
    "labor market", "labour market", "wage",
    "gig economy", "gig worker", "gig work", "platform worker",
    "freelancer", "remote work",
    # 算法治理（OB 视角）
    "algorithmic management", "algorithmic decision-making",
    "algorithmic hiring", "algorithm aversion", "algorithm appreciation",
    "responsible AI", "AI ethics", "AI governance",
    "trust in AI", "human-AI trust", "human-AI collaboration",
    "human-AI teaming", "AI-augmented work",
    # 组织行为（强信号）
    ...
    "marketing strategy", "brand management", "willingness to pay",
    "advertising effectiveness", "personalized recommendation",
    # 战略与创业（强信号）
...
def fetch_category(
    ...
    """生成器：按分类抓取与 OB 相关的 arXiv 论文。

    arXiv 的 query 长度有限制（~6000 字符），所以关键词分块发请求。
    每块用 cursor pagination。
    """
    # 把关键词切成 ~10 个一块，避免 URL 过长
    chunks = [OB_KEYWORDS[i : i + 10] for i in range(0, len(OB_KEYWORDS), 10)]
