...
def insert_raw_record(
    ...
    *,
    ...
    """Insert one record without endangering the surrounding transaction.

    Returns ``False`` only when the source record already exists. The insert is
    flushed inside a savepoint so a duplicate cannot roll back earlier records.
    """
    try:
